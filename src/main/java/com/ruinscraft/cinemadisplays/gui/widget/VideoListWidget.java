...
public class VideoListWidget extends ElementListWidget<VideoListWidgetEntry> {

    private final Supplier<VideoList> videoListSupplier;
    @Nullable
    private String search;

    public VideoListWidget(Supplier<VideoList> videoListSupplier, MinecraftClient client, int width, int height, int top, int bottom, int itemHeight) {
        super(client, width, height, top, bottom, itemHeight);
        this.videoListSupplier = videoListSupplier;
    ...
    public void update() {
        List<VideoListEntry> entries = videoListSupplier.get().getVideos();

        if (search != null)
            entries.removeIf(entry -> !entry.getVideoInfo().getTitle().toLowerCase(Locale.ROOT).contains(search));

        List<VideoListWidgetEntry> widgetEntries = entries.stream()
                .map(entry -> new VideoListWidgetEntry(entry, client))
                .sorted()
                .collect(Collectors.toList());

        replaceEntries(widgetEntries);
    }
